...
const sections = [
  ...
  { id: 'copilot', title: '① 伴走型——作業を渡す' },
  { id: 'workflow', title: '② 固定ワークフロー——手順を渡す' },
  { id: 'agent', title: '③ 委任型エージェント——目的を渡す' },
...
const figures = {
  ...
  fig3: {
    ...
    alt: '伴走型の実例：提案資料Skillの仕組み図',
    caption: '伴走型の実例——提案資料Skill',
  },
  fig4: {
    src: '/assets/three-levels/fig4.svg',
    alt: '固定ワークフローの実例：週次レポートラインの仕組み図',
    caption: '固定ワークフローの実例——週次レポートライン',
  },
  fig5: {
    src: '/assets/three-levels/fig5.svg',
    alt: '委任型エージェントの実例：新規開拓エージェントの仕組み図',
    caption: '委任型エージェントの実例——新規開拓エージェント',
  },
};
...
const faqs = [
  {
    q: '全部、委任型エージェントにするのが一番進んでいるのでは？',
    a: 'いいえ。型は業務の性質で決まるもので、優劣ではありません。判断が要る業務は今後も伴走型が正解ですし、定型業務は固定ワークフローの方が委任型エージェントより安く、壊れにくい。「全部エージェント」はコストと不安定さを抱え込むだけです。',
  },
  {
    q: 'どの型から始めるべきですか？',
    a: '固定ワークフローに載せられる定型業務（定時レポートなど）が最初の候補です。工数削減を実測しやすく、社内の合意形成に使える数字が最短で手に入ります。並行して、判断系の業務を伴走型で回しながら手順の言語化を進めるのが定石です。',
  },
  {
    q: '型の判定を間違えたらどうなりますか？',
    a: '深く渡しすぎた場合（定型業務を委任型エージェントにした等）は、挙動の不安定さとコスト高として現れます。浅くしか渡していない場合（定型業務を伴走型のままにした等）は、工数が減らないままです。いずれも判定をやり直して組み替えれば済む話で、だからこそ最初の業務分解と判定が重要になります。',
  },
  {
    q: 'ツール選定とはどう関係しますか？',
    a: '型が決まってからツールを選びます。伴走型ならSkillを装着できるAI（Claude等）、固定ワークフローならパイプライン基盤（Dify等）、委任型エージェントなら既製のエージェント環境から。ツールから入ると、ツールにできることに業務を合わせる本末転倒が起きます。',
  },
];
      ...
      <main>
        <article className="article">
          <header className="article-hero" id="top">
            ...
            <p className="article-lead">
              この記事では、この「AI委譲の3段階」——①伴走型（作業を渡す）／②固定ワークフロー（手順を渡す）／③委任型エージェント（目的を渡す）——の考え方と、それぞれの実例を解説します。
            </p>
          ...
          <div className="article-body">
            ...
            <section id="criteria">
              ...
              <ul className="article-list">
                <li>
                  <strong>判断が要る。作業しか渡せない</strong> → ① 伴走型
                </li>
                <li>
                  <strong>やり方を説明できる。手順まで渡せる</strong> → ② 固定ワークフロー
                </li>
                <li>
                  <strong>過程はどうでもいい。目的だけで足りる</strong> → ③ 委任型エージェント
                </li>
              </ul>
              <p>
            ...
            <section id="copilot">
              ...
              <p>
                <strong>この型の落とし穴は、Skill化をサボること。</strong> 毎回ゼロからプロンプトを書くと、品質が人によってブレ、組織の資産になりません。伴走型の成否は「AIの上手な使い方を個人が覚える」ことではなく、「業務の型をSkillとして言語化し、誰が使っても同じ品質が出る状態を作る」ことにあります。
              </p>
            </section>

            <section id="workflow">
              <h2>② 固定ワークフロー——手順を渡す</h2>
              <p>
            ...
            <section id="fit">
              ...
              <p>
                ただし、配分は固定ではありません。人がAIと往復するうちに、「この業務、毎回同じ指示をしているな」という工程が見えてきます。それは手順が言語化された合図で、固定ワークフローに降ろせます。ゴール条件まで明確に定義できた業務に限って、委任型エージェントに渡す選択肢が生まれます。<strong>伴走型での往復は、次の型のための言語化プロセスを兼ねている</strong>のです。
              </p>
              <p>
